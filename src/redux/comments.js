...
import {COMMENTS} from '../shared/comments';  //imported to campsites.js reducer
...
//now ^^^^ calling reducer to update part of the state, when the AddComment action is dispatched to the store. this mens we need to import from the actionTypes module done w/ using * wildcard syntax. 
...
export const Comments = (state = COMMENTS, action) => {
    ...
    switch (action.type) { //Using switch to add an action behaviour (adding comment in this case). Switch is not required but commonly used. right now we don't have action type defined so we use a default case to return the state: 
        case ActionTypes.ADD_COMMENT:    //setting up a case to when actionTypes is ADDCOMMENT
            const comment = action.payload;   //here is the content of action payload into a new variable called comment. since we recall content of action payload is an object so we can add more properties to this objects like following:
            comment.id = state.length;    //comment.id is the length of the comments array that's stored in this part of the state, 
            comment.date = new Date().toISOString();    //and we'll add today's date
            return state.concat(comment);   //finally retrn new state by array concat method , concat is js method and allows us to attach new item to end of an array, without muttating original array. it creates new array. in contrast, push() would mutate the original array so it cannot be used.
            //^^^ so this line takes an existing state, which is an array of objets and concatenates the new comment object, to the end of the array, and it returns a new state, to the redux store. 
            //Next, we update several react comments to enable dispatching this action.
        default:
